...
function Navbar({ hidden, setHidden }) {
  return (
    <div className="max-w-[1320px] flex items-center mx-auto justify-between gap-[20px]">
      <div>
        <h2 className="text-white font-bold text-[32px]">NICKNAME</h2>
      </div>
      ...
      <div>
        <button onClick={() => setHidden(true)} className="bg-white text-black shadow-lg py-[10px] hover:text-white cursor-pointer px-[50px] rounded-2xl font-medium hover:bg-inherit hover:outline hover:outline-white transition-all duration-300">
          Add
        </button>
      </div>
      {hidden && (
        <div>
          <Overlay setHidden={setHidden} />
          <AddModule setHidden={setHidden} />
        </div>
      )}
    </div>
  );
